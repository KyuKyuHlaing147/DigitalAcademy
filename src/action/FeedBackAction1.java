...
public class FeedBackAction1 extends ActionSupport{
	String username;
	String email;
	String course;
	String feedback;
...
public String execute() throws Exception{
	String sql="insert into feedback1(username,email,course,feedback) values (?,?,?,?)";
	
	PreparedStatement pre=db.getDBConnect().prepareStatement(sql);
	pre.setString(1, "Guest");
...
public String showFeedback() throws Exception{
	...
	String sql="select * from feedback1 where course='"+course1+"'";
	...
	session.setAttribute("flist", feb);
